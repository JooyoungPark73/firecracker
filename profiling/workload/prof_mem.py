...
def getMemStats(pid):
   ...
   Data_lines = []
   Text_lines = []
   Rss = (int(open(f'/proc/{pid}/statm').readline().split()[1]) * PAGESIZE)
   Swap_lines = []
   Swap_pss_lines = []

   Swap = 0

   if os.path.exists(f'/proc/{pid}/smaps'):  # stat
       smaps = 'smaps'
       if os.path.exists(f'/proc/{pid}/smaps_rollup'):
           smaps = 'smaps_rollup'  # faster to process
       lines = open(f'/proc/{pid}/{smaps}').readlines()  # open
       ...
       Private += Private_huge  # Add after as PSS doesn't account for huge pages
       ...
       Data = sum([int(line.split()[1]) for line in Data_lines])
       Instructions = sum([int(line.split()[1]) for line in Text_lines])
       ...
       Shared = int(open(f'/proc/{pid}/statm').readline().split()[2])
       Shared *= PAGESIZE
       Shared_huge = 0
       Private = Rss - Shared
       Data = 0
   ...
   print(Data)
   print(Instructions)
   return (Private, Shared, Shared_huge, Swap, mem_id, Data, Instructions)
