...
fn main() {
    ...
    // Spawn a thread to print the page fault counter every one second
    let page_fault_counter_print_clone = Arc::clone(&page_fault_counter);
    thread::spawn(move || {
        loop {
            thread::sleep(Duration::from_secs(1));
            println!("Pages served: {}", page_fault_counter_print_clone.load(Ordering::SeqCst));
        }
    });
