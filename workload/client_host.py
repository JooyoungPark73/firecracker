...
def main_benchmark(mmap_path='/dev/shm/nexus_region', mmap_size=16*1024*1024, num_iterations=10):
    ...
    # For host-initiated vsock connections, host connects to AF_UNIX socket at uds_path
    vsock_socket_path = "/tmp/v.sock"
    
    # Open the symlink - it points to /proc/<fc-pid>/fd/<memfd>
    # This transparently gives us access to the memfd hugepage memory
    f = os.open(mmap_path, os.O_RDWR)
    s = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
    try:
        # mmap the memfd (no need for MAP_HUGETLB flag, it's already hugepage-backed)
        mm = mmap.mmap(
        f, 
        mmap_size,
        offset=0, 
        ...
        print(f"Connecting to {vsock_socket_path}...")
        s.connect(vsock_socket_path)
        
        # Send CONNECT command to establish connection to guest port
        connect_cmd = f"CONNECT {PORT}\n"
        s.sendall(connect_cmd.encode('ascii'))
        print(f"Sent: {connect_cmd.strip()}")
        
        # Read acknowledgement "OK <port>\n"
        response = b""
        while b'\n' not in response:
            chunk = s.recv(1024)
            if not chunk:
        ...
        for size in test_sizes:
            ...
            for i in range(num_iterations):
                ...
                s.sendall(b'1' if verified else b'0')
